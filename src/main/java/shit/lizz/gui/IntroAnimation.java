...
extends ClientBase {
    ...
    public void onRender(GlRenderEvent glRenderEvent) {
        ...
        float zWidth = GlHelper.getStringWidth("L", scaledZFont);
        float enWidth = GlHelper.getStringWidth("IZZ", baseFont);
        ...
        GlHelper.drawText("L", zRenderX, zRenderY, scaledZFont, zColor);
        if (enAlpha > 0.0f) {
            ...
            GlHelper.drawText("IZZ", enX, enY, baseFont, enColor);
        }
    }
    ...
    private void finish() {
        if (!this.finished) {
            this.finished = true;
            try {
                LizzClient.instance.getEventBus().unregister(this);
