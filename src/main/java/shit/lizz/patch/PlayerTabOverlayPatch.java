...
import shit.lizz.event.impl.ChatReceiveEvent;
import shit.lizz.hud.TabListInfo;
...
public class PlayerTabOverlayPatch extends ClientBase {
    ...
    public static void onRenderPre(PlayerTabOverlay overlay, GuiGraphics graphics, int width, Scoreboard scoreboard, Objective objective, CallbackInfo callbackInfo) {
        ...
        if (!LizzClient.isReady() || LizzClient.getInstance().getModuleManager() == null) return;
        Watermark watermark = (Watermark) LizzClient.getInstance().getModuleManager().getModule(Watermark.class);
