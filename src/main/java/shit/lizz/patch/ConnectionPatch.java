...
import shit.lizz.event.impl.PrePacketEvent;
...
public class ConnectionPatch extends ClientBase {
    public static boolean onPacketReceive(Packet<?> packet) {
        if (mc == null || mc.level == null || mc.player == null || packet == null || !LizzClient.isReady()) {
            return false;
        }
        PrePacketEvent prePacket = new PrePacketEvent(packet);
        LizzClient.getInstance().getEventBus().call(prePacket);
