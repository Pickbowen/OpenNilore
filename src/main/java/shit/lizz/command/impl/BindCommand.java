...
extends Command {
    public static final class EventHandler {
        ...
        @EventTarget
        public void onKey(shit.lizz.event.impl.KeyEvent event) {
            ...
            LizzClient.getInstance().getEventBus().unregister(this);
            LizzClient.getInstance().getConfigManager().saveAll();
        }
    }
    ...
    private void handleBindInteractive(String moduleName) {
        try {
            Module module = LizzClient.getInstance().getModuleManager().getModule(moduleName);
            if (module == null) {
                ChatUtil.print("Invalid module.");
                return;
            }
            ChatUtil.print("Press a key to bind " + moduleName + " to.");
            LizzClient.getInstance().getEventBus().register(new BindCommand.EventHandler(this, module, moduleName));
