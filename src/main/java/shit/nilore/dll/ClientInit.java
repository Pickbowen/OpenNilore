 ...
 * jar onto the Forge GameClassLoader, so {@link NiloreClient} and all 21 patch
 * handler classes share Minecraft's class loader.
 *
 * <p>This entry point intentionally does <b>not</b> construct {@link NiloreClient}.
 ...
 * NiloreClient} on the next tick. Letting the existing tick-driven lazy-init run
 * keeps the DLL path and the mod path identical from {@code NiloreClient.init}
 * onwards.</p>
 */
public final class ClientInit {
    ...
    public static synchronized void start(String extractedJarPath) {
        ...
        try {
            ...
            LOGGER.info("NiloreClient loader     = {}", NiloreClient.class.getClassLoader());
            ...
            // performs the real NiloreClient construction via its existing
            // lazy-init path.
            NiloreClient.registerPatches();
            ClassAgent.installPatchesAndRetransform();

            LOGGER.info("ClientInit done. NiloreClient will be constructed on the next Minecraft.tick.");
        } catch (Throwable t) {
