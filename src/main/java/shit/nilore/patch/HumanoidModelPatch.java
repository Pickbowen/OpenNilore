...
public class HumanoidModelPatch {
    public static CameraPitchEvent onPitchRender(LivingEntity entity, float pitch) {
        if (NiloreClient.isReady() && entity == ClientBase.mc.player && ClientBase.mc.level != null) {
            return (CameraPitchEvent) NiloreClient.instance.getEventBus().call(new CameraPitchEvent(pitch));
        }
